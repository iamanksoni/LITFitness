package com.litmethod.android.ui.Dashboard.AllClassTabScreen.CoverScreen.ClassesCoverScreen
...
import com.litmethod.android.databinding.TrainnerProfileViewBinding
...
import com.litmethod.android.ui.Dashboard.AllClassTabScreen.CoverScreen.TrainerProfileScreen.TrainerProfileScreenActivity
...
//        setUpAdapter()
    ...
    private fun setUpUi() {
        equipMentVideoList = AllClassesDataObject.getClassDetailsResponse.equipment_video as ArrayList<EquipmentVideo>
        instructorInfoist = AllClassesDataObject.getClassDetailsResponse.instructor_info as ArrayList<InstructorInfo>
        ...
        if(AllClassesDataObject.getClassDetailsResponse.isSave){
            isVideoSave = true
        ...
        if (AllClassesDataObject.getClassDetailsResponse.isViewed== false){
        ...
        val date: Date = originalFormat.parse(AllClassesDataObject.getClassDetailsResponse.date)
        val formattedDate: String = targetFormat.format(date)
        binding.tvDate.text = "Aired On $formattedDate"

        Glide.with(this)
            .load(AllClassesDataObject.getClassDetailsResponse.thumbnail)
            .into(binding.expandedImage)
        Glide.with(this)
            .load(item.instructor_image)
            .into(binding.trainnerProfileSub.ivInstructorImage)

        val firstWord = "${AllClassesDataObject.getClassDetailsResponse.title}"
        val secondWord = " with ${AllClassesDataObject.getClassDetailsResponse.getInstructor}"
        ...
        val firstArray = AllClassesDataObject.getClassDetailsResponse.equipment.map {
            it.name
        }
        val secondArray = AllClassesDataObject.getClassDetailsResponse.accessories.map {
            it.name
        }
        val combinedEquipments = (firstArray + secondArray).joinToString()
         binding.tvCombinedEquip.text = combinedEquipments
        val firstWordtime = "${AllClassesDataObject.getClassDetailsResponse.getDurations}"
        val secondWordtime = "  •  "
        val ThirdWordtime = "${AllClassesDataObject.getClassDetailsResponse.getLevelName}"
        val FourthWordtime = "  •  "
        val  FifthWordtime= "${AllClassesDataObject.getClassDetailsResponse.class_type}"
        ...
        when (AllClassesDataObject.getClassDetailsResponse.getLevelName){
            "Intermediate" -> {
                spannabletime.setSpan(
    ...
    private fun progressBarAnimation() {
        val timer = object : CountDownTimer(1000, 500) {
            ...
            override fun onFinish() {
              val progressLikePercent=  AllClassesDataObject.getClassDetailsResponse.classRatting.totalLikepersentage.toFloat()
                val totalRating =AllClassesDataObject.getClassDetailsResponse.classRatting.totalCount
                 val difficultyRating = AllClassesDataObject.getClassDetailsResponse.difficultyRating
...
//                binding.trainnerProgressSub.trpRating2.progress = 70f
            ...
            R.id.imageView -> {
                ...
                if(isVideoSave){
                    viewModel.checkgetClassBookmark(AllClassesDataObject.accessToken,
                        ClassBookmarkRequest("classBookmark","${AllClassesDataObject.getClassDetailsResponse.id}","unsave")
                    )
                    isVideoSave=false
                    binding.imageView.setImageResource(R.drawable.ic_like)
                }else{
                    viewModel.checkgetClassBookmark(AllClassesDataObject.accessToken,
                        ClassBookmarkRequest("classBookmark","${AllClassesDataObject.getClassDetailsResponse.id}","save")
                    )
                    isVideoSave=true
                    binding.imageView.setImageResource(R.drawable.ic_star_active)
                }
    ...
    private  fun loginResponse(){
        ...
        viewModel.getInstructorInfoResponse.observe(this, Observer {
            ...
            AllClassesDataObject.getInstructorInfoResponse = it.result.pagenation.data
