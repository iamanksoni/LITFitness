...
class GoalsAdapter(
    val result: ArrayList<GoalsData>,
    val context: Context,
    val getGoalData:List<Data>
) : RecyclerView.Adapter<GoalsAdapterViewHolder>() {
    lateinit var goalsAdapterListener: GoalsAdapterListener
    ...
    interface GoalsAdapterListener {
        fun onItemClick(position: Int,data:String)
    }
    fun setAdapterListenerNew(goalsAdapterListener: GoalsAdapterListener) {
        this.goalsAdapterListener = goalsAdapterListener
    }
...
//        holder.tv_header1.text = "Recovery"
//        holder.iv_level.setImageDrawable(context.getDrawable(R.drawable.iv_recovery))
        ...
        holder.itemView.setOnClickListener {
            if (position==result.size-1){
                goalsAdapterListener.onItemClick(position,"999")
            } else{
                goalsAdapterListener.onItemClick(position,item!!.id)
            }
