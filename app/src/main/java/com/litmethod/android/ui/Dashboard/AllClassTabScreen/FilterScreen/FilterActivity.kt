...
import com.litmethod.android.ui.Dashboard.AllClassTabScreen.FilterScreen.ViewModel.FilterActivityViewModel
import com.litmethod.android.ui.Dashboard.AllClassTabScreen.FilterScreen.ViewModel.FilterActivityViewModelFactory
import com.litmethod.android.ui.Dashboard.AllClassTabScreen.FilterScreen.model.ChildFilterActivityResponse
...
class FilterActivity : BaseActivity(), View.OnClickListener,FilterChildAdapter.FilterChildAdapterListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        clickListener()
        viewModel.checkGetFilterList(AllClassesDataObject.accessToken, FilterListRequest("filterList"))
    ...
    private fun  setArryData(){
        if (AllClassesDataObject.isFilter==true){
            level = AllClassesDataObject.level
            duration = AllClassesDataObject.duration
            equipMent = AllClassesDataObject.equipMent
            accessorries = AllClassesDataObject.accessorries
            instrurtor = AllClassesDataObject.instrurtor
            muscleGroup = AllClassesDataObject.muscleGroup
        }
    }
    private fun setUpUi() {
    ...
    override fun onClick(p0: View?) {
        ...
        when (p0!!.id) {
            ...
            R.id.btn_apply_filters ->{
                AllClassesDataObject.filterToClaassFragment = true
                AllClassesDataObject.level = level
                AllClassesDataObject.equipMent = equipMent
                AllClassesDataObject.instrurtor = instrurtor
                AllClassesDataObject.duration = duration
                AllClassesDataObject.accessorries = accessorries
                AllClassesDataObject.muscleGroup = muscleGroup
                AllClassesDataObject.termId = codeId
                AllClassesDataObject.isFilter = true
                finish()
            }
            R.id.cv_taken_by_me ->{
            ...
            R.id.ib_reset ->{
                AllClassesDataObject.filterToClaassFragment = true
                AllClassesDataObject.level.clear()
                AllClassesDataObject.equipMent.clear()
                AllClassesDataObject.instrurtor.clear()
                AllClassesDataObject.duration.clear()
                AllClassesDataObject.accessorries.clear()
                AllClassesDataObject.muscleGroup.clear()
                AllClassesDataObject.termId = codeId
                AllClassesDataObject.isFilter = false
                finish()
            }
...
//                    for (j in 0 until AllClassesDataObject.level.size) {
//                        if (dataListChild[i].childData.attributeCode == AllClassesDataObject.level[j]) {
...
//                dataList.add(FilterActivityData("${it.name}",dataListChild,false))
                it.parameter.map {
                    var isAddData = false
                    if (AllClassesDataObject.isFilter == true){
                       for (i in 0..AllClassesDataObject.level.size-1){
                           var data = AllClassesDataObject.level[i]
                ...
                "typeduration" ->{
                    ...
                    it.parameter.map {
                        var isAddData = false
                        if (AllClassesDataObject.isFilter == true){
                            for (i in 0..AllClassesDataObject.duration.size-1){
                                var data = AllClassesDataObject.duration[i]
                                if (it.attributeCode==data){
                                    isAddData = true
                ...
                "typeMuscleGroup" ->{
                    it.parameter.map {
                        ...
                        var isAddData = false
                        if (AllClassesDataObject.isFilter == true){
                            for (i in 0..AllClassesDataObject.muscleGroup.size-1){
                                var data = AllClassesDataObject.muscleGroup[i]
                                if (it.attributeCode==data){
                                    isAddData = true
                ...
                "typeEquipment" ->{
                    it.parameter.map {
                        ...
                        var isAddData = false
                        if (AllClassesDataObject.isFilter == true){
                            for (i in 0..AllClassesDataObject.equipMent.size-1){
                                var data = AllClassesDataObject.equipMent[i]
                                if (it.attributeCode==data){
                                    isAddData = true
                ...
                "typeAccessories" ->{
                    it.parameter.map {
                        ...
                        var isAddData = false
                        if (AllClassesDataObject.isFilter == true){
                            for (i in 0..AllClassesDataObject.accessorries.size-1){
                                var data = AllClassesDataObject.accessorries[i]
                                if (it.attributeCode==data){
                                    isAddData = true
                ...
                "instructor"->{
                      it.parameter.map {
                          ...
                          var isAddData = false
                          if (AllClassesDataObject.isFilter == true){
                              for (i in 0..AllClassesDataObject.instrurtor.size-1){
                                  var data = AllClassesDataObject.instrurtor[i]
                                  if (it.attributeCode==data){
                                      isAddData = true
